...
function SubjectView({currentSubject, subjects, isEdit, setIsEdit}){
    ...
    function setPriorityColor(task){
        if(task.isCompleted){
            return "bg-teal-500 bg-opacity-50 shadow-lg rounded-xl my-5 py-2 pl-2 grid grid-cols-7 items-center"
        }
    ...
    return(
        <>
                ...
                <div className="bg-white bg-opacity-20 shadow-xl m-10 px-5 py-2 rounded-xl">
                    <div className="text-white py-2 flex justify-end items-center">
                        ...
                        <button 
                            ...
                            className="flex justify-between w-[13%] items-center text-lg bg-red-400 shadow-red-400/50 p-2 mx-5 rounded-xl shadow-md"
                        >
                            <MdOutlineDeleteForever />Delete collection
                        </button>
                    </div>
            ...
            {taskPop ? <TaskPop setTaskPop={setTaskPop} task={taskToView} subjects={subjects} isEdit={isEdit} setIsEdit={setIsEdit} /> : null}
            {isSubjectDelete ? <SubjectDeletePop setIsSubjectDelete={setIsSubjectDelete} currentSubject={currentSubject} tasks={tasksBySubject} /> : null}
        </>
    )
}
